...
function timeSpanToString(startDate, endDate) {
  ...
  milliseconds = (milliseconds.toString().length < 2)
    ? `00${milliseconds}` : milliseconds;
...
function angleBetweenClockHands(date) {
  ...
  let result = (Math.abs(hourAngle.toFixed(2) - minuteAngle.toFixed(2)) / 180) * Math.PI;
  if (result > Math.PI) {
    result = ((Math.abs(hourAngle.toFixed(2) - minuteAngle.toFixed(2)) / 180) * Math.PI) - Math.PI;
  }
  return result;
}
